...
export class PermisoComponent implements OnInit {
  @ViewChild(PermisoModalComponent) modal: PermisoModalComponent;

  entities$: Observable<Permiso[]> = this.store.select(permisoReducer.getAll);

  loading$: Observable<boolean> = this.store.select(permisoReducer.getLoading);

  count$: Observable<number> = this.store.select(permisoReducer.getCount);
  ...
  getAll() {
    ...
    this.store.dispatch(
      permisoActions.loadAll({ payload: this.requestFilter }),
    );
  }
      ...
      case 'delete': {
        this.confirmationSrv.confirm({
          message: this.translateSrv.instant('messages.confirmation.message', {
            name: this.translateSrv.instant('tables.permiso.singular'),
          }),
          ...
          accept: () => {
            this.store.dispatch(permisoActions.delete({ id: event.data.id }));
          },
        });
        break;
      }
